...
function NplRow({row, set_row, remove_row}) {
    ...
    return <tr className="table-info result-row">
        <td className="result-cell-action" data-label="操作"><a className="btn btn-sm btn-outline-primary ssmall text-nowrap mineralize-btn"
               onClick={remove_row}>删除</a></td>
        <td className="result-cell-demand" data-label="需求">
        ...
        <td className="result-cell-factory" data-label="工厂">
        ...
        {/* 所选配方 */}
        <td className="result-cell-recipe" data-label="配方"><RecipeSelect item={item}
                          choice={row["配方id"]}
        ...
        {/* 所选增产模式 */}
        <td className="result-cell-pro-mode" data-label="增产模式"><ProModeSelect recipe_id={recipe_id} choice={row["增产模式"]} onChange={set_row_prop("增产模式", true)}/>
        </td>
        {/* 所选增产剂 */}
        <td className="result-cell-pro-num" data-label="增产剂">{row["增产模式"] !== 0 &&
            <ProNumSelect choice={row["增产点数"]} onChange={set_row_prop("增产点数", true)}/>}</td>
        <td className="result-cell-factory-type" data-label="工厂类型">
            {/* 所选工厂种类 */}
