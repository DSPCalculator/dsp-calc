...
import default_game_data from './gamedata.js'
...
window.resetNeeds = function (item) {
...
window.changeNeeds = function (item) {
...
window.if_proliferate_itself = function (proliferate_itself) {
...
window.changeFixedNum = function () {
    fixed_num = document.getElementById("显示精度").value;
    if (fixed_num <= 0) fixed_num = 0;
    show_result_dict();
}//更改显示精度

window.BatchChangeFactoryOf = function (factory) {
...
window.BatchChangeProMode = function () {
...
window.BatchChangeProNum = function () {
...
window.NPChangeSchemeOf = function (i) {
...
window.NPDeleteLine = function (i) {
...
window.NPChangeItem = function (i) {
...
window.changeFractionatingSpeed = function () {
...
window.IfEnergyContainMiner = function () {
...
const dom_result = ReactDOM.createRoot(document.getElementById('result'));
...
function show_result_dict() {
    ...
    for (let i in result_dict) {
        ...
        doms.push(<tr key={i} id={`row_of_${i}`}>
            {/* //操作 */}
            <th><button className='btn btn-sm btn-outline-primary' onClick={() => mineralize(i)}>视为原矿</button></th>
            {/* //目标物品 */}
            <th><img src={`./image/${i}.png`} title={i} style={{ width: '40px', height: '40px' }} /></th>
            {/* //分钟毛产出 */}
            <th id={`num_of_${i}`}>{get_gross_output(result_dict[i], i).toFixed(fixed_num)}</th>
            {/* //所需工厂*数目 */}
    ...
    dom_result.render(<>{doms}</>);
    // document.getElementById("result").innerHTML = str;

    // TODO removed but not implemented
...
function build_item_list() {
    function delete_item_from_product_graph(name) {
        for (var item in product_graph[name]["原料"]) {
            delete product_graph[item]["可生产"][name];
        }
        for (var item in product_graph[name]["可生产"]) {
...
window.unmineralize = function (item) {
