...
import type {GameData, NumericMap, ProliferatorPrice, RecipeData, RecipeScheme, Settings} from '../../types/domain';
...
function scale_item_dict(dict: NumericMap, multiplier: number): NumericMap {
    if (multiplier === 1) {
        return dict;
    }
    const scaled: NumericMap = {};
...
function cancel_shared_items(recipe: RecipeData): void {
...
export function get_factory_speed_multiplier(factory_name: string, target_item: string, settings: Settings): number {
    if (factory_name === "采矿机") {
...
export function get_equivalent_recipe_output_rate(recipe: RecipeData, target_item: string): number {
