import {get_equivalent_recipe_output_rate, get_factory_speed_multiplier} from './equivalentRecipe.js';
import {hasMineralizedItem} from './mineralizeState.js';

export function buildNormalizedSchemeData(game_data, scheme_data) {
    let normalized_scheme_for_recipe = scheme_data.scheme_for_recipe.map(recipe_setting => ({...recipe_setting}));
...
export function buildProliferatorPrice(game_data, proliferate_itself) {
...
export function buildItemList(item_graph) {
    const product_graph = JSON.parse(JSON.stringify(item_graph));
    const item_list = [];
    const key_item_list = [];
    ...
    function find_item(name, isProduction) {
        if (!isProduction) {
