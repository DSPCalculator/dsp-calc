...
*/
const data_index_modules = import.meta.glob(['../../../data/*.json', '!../../../data/Vanilla.json'], {
    import: 'default',
});
const data_indices = {
    Vanilla: VanillaData,
...
function build_fractionate_outputs(output_infos, get_item_by_id) {
    if (!output_infos) {
        return [];
    }
...
export const default_game_data = build_game_data([VanillaGUID], VanillaData);
export const vanilla_game_version = game_data_info_list[0].version;

var name_icon_list;

function get_json_file_name(mod_guid_list) {
...
function build_game_data(mod_guid_list, json_data) {
    let data = {};
    ...
    let json_file_name = get_json_file_name(mod_guid_list);
    //将json转换为需要的数据结构
...
async function load_json_data(json_file_name) {
...
export async function get_game_data(mod_guid_list) {
    let normalized_mod_guid_list = mod_guid_list.length === 0 ? [VanillaGUID] : mod_guid_list;
