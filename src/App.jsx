...
const BatchSetting = lazy(() => import('./batch_setting.jsx').then(module => ({default: module.BatchSetting})));
const Result = lazy(() => import('./result.jsx').then(module => ({default: module.Result})));
const Settings = lazy(() => import('./settings.jsx').then(module => ({default: module.Settings})));
...
function GameVersion({needs_list, set_needs_list}) {
    ...
    async function mods_change(modList) {
        ...
        set_mods(modList2);
        let game_data = await get_game_data(modList);
